# ============================================================
# Класифікація наявності пандусів — ramp vs no_ramp
# Адаптовано для Google Cloud Vertex AI + GCS
#
...
matplotlib.use('Agg')  # без GUI для серверного середовища
...
# Назви класів та їх мітки
CLASS_DIRS = {
    'ramp': 1,
    'no_ramp': 0,
}
CLASS_NAMES = ['no_ramp', 'ramp']
...
# ── Локальна тимчасова папка для моделей ─────────────────────
LOCAL_MODELS_DIR = tempfile.mkdtemp()
print(f'Локальна папка моделей: {LOCAL_MODELS_DIR}')


# ── Завантаження файлу з GCS в пам'ять ───────────────────────
def download_blob_to_bytes(blob_name):
    blob = bucket.blob(blob_name)
    return blob.download_as_bytes()


# ── Крок 1: Збір файлів з GCS ────────────────────────────────
print('\nЗбираємо файли з GCS...')
all_files = []

for class_name, label in CLASS_DIRS.items():
    prefix = f'{DATASET_PREFIX}/{class_name}/'
    blobs = list(storage_client.list_blobs(BUCKET_NAME, prefix=prefix))
    jpg_blobs = [b.name for b in blobs if b.name.lower().endswith('.jpg')]
...
# ── Крок 2: Перевірка битих файлів ───────────────────────────
def is_valid_gcs(blob_name):
    try:
...
with ThreadPoolExecutor(max_workers=8) as ex:
    valid = list(ex.map(is_valid_gcs, df['path']))

print(f'Битих: {sum(1 for v in valid if not v)}')
df['valid'] = valid
df = df[df['valid']].copy().reset_index(drop=True)
print(f'Залишилось: {len(df)}')


# ── Крок 3: Weighted loss ─────────────────────────────────────
...
class GCSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            data = download_blob_to_bytes(row['path'])
            with Image.open(io.BytesIO(data)) as img:
...
def make_loaders(df):
    ...
    train_loader = DataLoader(GCSDataset(train_df, train_transform),
                              batch_size=BATCH_SIZE, shuffle=True, num_workers=2)
    val_loader = DataLoader(GCSDataset(val_df, val_transform),
                            batch_size=BATCH_SIZE, shuffle=False, num_workers=2)
    test_loader = DataLoader(GCSDataset(test_df, val_transform),
                             batch_size=BATCH_SIZE, shuffle=False, num_workers=2)
...
def train_model(arch, n_epochs=NUM_EPOCHS):
    print(f'\n{"=" * 50}')
    print(f'  {arch.upper()} | ramp vs no_ramp (2 кл.)')
    print(f'{"=" * 50}')
...
for arch in ARCHS:
    ...
    # Перевіряємо чи вже є модель в GCS
    ...
    # Зберігаємо локально і завантажуємо в GCS
    torch.save(model.state_dict(), local_save_path)
    upload_to_gcs(local_save_path, gcs_save_path)

# Зберігаємо також найкращу модель як _best.pth (для ASP.NET)
if results:
    ...
    print(f'\n  ⭐ Найкраща модель: {best["model"]} (F1={best["f1"]})')

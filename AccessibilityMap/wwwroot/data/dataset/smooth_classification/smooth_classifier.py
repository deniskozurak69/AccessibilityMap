# ============================================================
# Класифікація нерівності тротуарів — severe / slight / smooth
# Адаптовано для Google Cloud Vertex AI + GCS (Кешування на диск)
#
# Структура в GCS bucket:
#   gs://BUCKET_NAME/
#     dataset/
#       smooth_classification/
#         severe/   <- значні руйнування / ями
#         slight/   <- невеликі тріщини / плитка з дефектами
#         smooth/   <- ідеальне покриття / новий асфальт
...
parser.add_argument('--dataset_path', type=str,
                    ...
                    help='gs://bucket/dataset/smooth_classification')
parser.add_argument('--output_path', type=str,
                    default=os.environ.get('OUTPUT_PATH', ''),
                    help='gs://bucket/models')
parser.add_argument('--model_name', type=str,
                    default=os.environ.get('MODEL_NAME', 'smooth'),
...
# ── Налаштування ─────────────────────────────────────────────
CLASSES = {'severe': 0, 'slight': 1, 'smooth': 2}
...
# Локальні папки на швидкому NVMe інстансу Vertex AI
LOCAL_MODELS_DIR = tempfile.mkdtemp()
LOCAL_DATA_DIR = os.path.join(tempfile.gettempdir(), 'smooth_dataset')
os.makedirs(LOCAL_DATA_DIR, exist_ok=True)

# ── Крок 1: Збір списку файлів з GCS ──────────────────────────
print('\nЗбираємо список файлів з GCS за класами...')
rows = []

for class_name, label in CLASSES.items():
    prefix = f'{DATASET_PREFIX}/{class_name}/'
    blobs = list(storage_client.list_blobs(BUCKET_NAME, prefix=prefix))
    valid_blobs = [b.name for b in blobs if b.name.lower().endswith(('.jpg', '.jpeg', '.png'))]
    print(f'  {class_name} ({label}): {len(valid_blobs)} фото')
    for blob_name in valid_blobs:
        rows.append({'gcs_path': blob_name, 'label': label})

df = pd.DataFrame(rows)
df = df.sample(frac=1, random_state=SEED).reset_index(drop=True)
print(f'Всього знайдено записів у датасеті: {len(df)}')
...
def train_model(arch, n_epochs=NUM_EPOCHS):
    print(f'\n{"=" * 50}')
    print(f'  {arch.upper()} | severe/slight/smooth (3 кл.)')
    print(f'{"=" * 50}')
...
for arch in ARCHS:
    ...
    # Перевіряємо наявність збереженої моделі в бакеті
    ...
    # Збереження поточної моделі
    torch.save(model.state_dict(), local_save_path)
    upload_to_gcs(local_save_path, gcs_save_path)

    # Очищення VRAM для запобігання OOM помилок
    del model
    torch.cuda.empty_cache()

# Експорт найкращих ваг як системного артефакту _best.pth
if results:
...
if results:
    ...
    # Візуалізація та відправка графіку порівняння моделей в GCS
    fig, ax = plt.subplots(figsize=(8, 4))
    results_df.plot(x='model', y=['accuracy', 'f1'], kind='bar', ax=ax, rot=0)
    ax.set_title(f'{MODEL_NAME.capitalize()}ness Classifier — порівняння моделей')
    ax.set_ylim(0, 1)
...
# Повне очищення простору контейнера Vertex AI від тимчасових локальних файлів
shutil.rmtree(LOCAL_DATA_DIR, ignore_errors=True)
print('\n✅ Навчання завершено, дані вивантажено в хмару!')

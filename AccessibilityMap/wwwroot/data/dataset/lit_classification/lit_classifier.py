# ============================================================
# Класифікація освітлення тротуарів — lit vs unlit
...
#     dataset/
#       lit_classification/
#         lit_sidewalks/    <- освітлені
#         unlit_sidewalks/  <- неосвітлені
...
parser.add_argument('--dataset_path', type=str,
                    ...
                    help='gs://bucket/dataset/lit_classification')
parser.add_argument('--output_path', type=str,
                    default=os.environ.get('OUTPUT_PATH', ''),
                    help='gs://bucket/models')
parser.add_argument('--model_name', type=str,
                    default=os.environ.get('MODEL_NAME', 'lit'),
...
# Назви класів (папок на GCS) та їх мітки
CLASS_DIRS = {
    'lit_sidewalks': 1,
    'unlit_sidewalks': 0,
}
CLASS_NAMES = ['unlit', 'lit']
...
# Локальні тимчасові папки на NVMe/SSD контейнера Vertex AI
LOCAL_MODELS_DIR = tempfile.mkdtemp()
LOCAL_DATA_DIR = os.path.join(tempfile.gettempdir(), 'lit_dataset')
...
def download_and_validate(row_tuple):
    ...
    try:
        ...
        # Зберігаємо локально для миттєвого читання під час навчання
...
# ── Крок 4: Локальний Dataset (Швидке читання) ────────────────
...
def make_loaders(df):
    ...
    # Використовуємо велику кількість num_workers та pin_memory завдяки локальним файлам
    train_loader = DataLoader(LocalDataset(train_df, train_transform),
                              batch_size=BATCH_SIZE, shuffle=True, num_workers=4, pin_memory=True)
    val_loader = DataLoader(LocalDataset(val_df, val_transform),
                            batch_size=BATCH_SIZE, shuffle=False, num_workers=4, pin_memory=True)
    test_loader = DataLoader(LocalDataset(test_df, val_transform),
                             batch_size=BATCH_SIZE, shuffle=False, num_workers=4, pin_memory=True)
...
def train_model(arch, n_epochs=NUM_EPOCHS):
    print(f'\n{"=" * 50}')
    print(f'  {arch.upper()} | lit vs unlit (2 кл.)')
    print(f'{"=" * 50}')
...
# Копіюємо найкращу модель під окремим системним файлом (зручно для веб-сервісів)
if results:

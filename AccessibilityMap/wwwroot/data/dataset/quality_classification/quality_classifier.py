# ============================================================
# Класифікація якості поверхні — Mapillary датасет (surface_quality)
# Адаптовано для Google Cloud Vertex AI + GCS (Кешування на disk)
#
# Структура в GCS bucket:
#   gs://BUCKET_NAME/
#     mapillary_surface/
#       data.csv         <- файл метаданих з колонками mapillary_image_id та surface_quality
...
parser.add_argument('--model_name', type=str,
                    default=os.environ.get('MODEL_NAME', 'quality'),
...
CLASS_NAMES = list(SURFACE_QUALITY_MAP.keys())
...
LOCAL_DATA_DIR = os.path.join(tempfile.gettempdir(), 'quality_dataset')
...
# Фільтруємо лише записи, які мають валідну мітку якості
df = df[df['surface_quality'].isin(SURFACE_QUALITY_MAP.keys())].copy()
df['filename'] = df['mapillary_image_id'].astype(str) + '.jpg'
df['label'] = df['surface_quality'].map(SURFACE_QUALITY_MAP)

print(f'Записів після фільтрації за якістю: {len(df)}')
...
def download_and_validate_image(row_tuple):
    ...
    try:
        ...
        # Рятуємо швидкість тренування: пишемо дані на SSD контейнера
...
# Передаємо кортеж (index, filename, label) для економії пам'яті в ThreadPool
...
print(df['surface_quality'].value_counts().to_string())
...
def make_loaders(df_data):
    ...
    # Використовуємо pin_memory=True та num_workers=4, оскільки I/O тепер локальний і миттєвий
...
def train_model(arch, df_data, num_classes=5, n_epochs=NUM_EPOCHS):
    print(f'\n{"=" * 50}')
    print(f'  {arch.upper()} | surface_quality ({num_classes} кл.)')
    print(f'{"=" * 50}')
...
for arch in ARCHS:
    ...
    # Ретельно звільняємо VRAM відеокарти перед ініціалізацією наступного бекбону
    del model
...
if results:
    ...
    print(f'\n  ⭐ Найкраща архітектура якості: {best["model"]} (F1={best["f1"]})')
...
if results:
    ...
    ax.set_title(f'{MODEL_NAME.capitalize()}ness Quality Classifier — порівняння моделей')
    ax.set_ylim(0, 1)
...
print('\n✅ Навчання за якістю покриття повністю завершено!')

# ============================================================
# Класифікація тактильної плитки — бінарна (yes/no)
...
#     dataset/
#       tactile/
#         images/            <- позитиви (є плитка, .png / .jpg)
#         tactile_no_images/ <- негативи (немає плитки, .jpg)
...
parser.add_argument('--dataset_path', type=str,
                    ...
                    help='gs://bucket/dataset/tactile')
parser.add_argument('--output_path', type=str,
                    default=os.environ.get('OUTPUT_PATH', ''),
                    help='gs://bucket/models')
parser.add_argument('--model_name', type=str,
                    default=os.environ.get('MODEL_NAME', 'tactile'),
...
# Локальні папки на швидкому SSD диск контейнера Vertex AI
LOCAL_MODELS_DIR = tempfile.mkdtemp()
LOCAL_DATA_DIR = os.path.join(tempfile.gettempdir(), 'tactile_dataset')
os.makedirs(LOCAL_DATA_DIR, exist_ok=True)


# ── Крок 1: Збір файлів з GCS та балансування ────────────────
def collect_gcs_blobs(prefix, extensions):
    blobs = list(storage_client.list_blobs(BUCKET_NAME, prefix=prefix))
    return [b.name for b in blobs if b.name.lower().endswith(extensions)]


print('\nЗбираємо список файлів з GCS...')
pos_prefix = f'{DATASET_PREFIX}/images/'
neg_prefix = f'{DATASET_PREFIX}/tactile_no_images/'
extensions = ('.png', '.jpg', '.jpeg')

pos_files = collect_gcs_blobs(pos_prefix, extensions)
neg_files = collect_gcs_blobs(neg_prefix, extensions)

print(f'Позитивів (yes): {len(pos_files)}')
print(f'Негативів (no):  {len(neg_files)}')

# Балансування — обмежуємо позитиви до кількості негативів
if len(pos_files) > len(neg_files):
    random.seed(SEED)
    pos_files = random.sample(pos_files, len(neg_files))
    print(f'Позитивів після балансування: {len(pos_files)}')

# Формуємо датафрейм метаданих
df = pd.DataFrame(
    [(f, 1) for f in pos_files] + [(f, 0) for f in neg_files],
    columns=['gcs_path', 'label']
).sample(frac=1, random_state=SEED).reset_index(drop=True)

print(f'Разом до завантаження: {len(df)} (yes={df["label"].sum()}, no={(df["label"] == 0).sum()})')
...
def download_and_validate(row_tuple):
    ...
    try:
        ...
        # Рятуємо швидкість навчання: пишемо дані на SSD контейнера
...
print('\nКешуємо та валідуємо файли локально...')
...
# ── Крок 3: Локальний Dataset (Швидке читання) ────────────────
class LocalTactileDataset(Dataset):
...
def make_loaders(df):
    ...
    train_loader = DataLoader(LocalTactileDataset(train_df, train_transform),
                              batch_size=BATCH_SIZE, shuffle=True, num_workers=4, pin_memory=True)
    val_loader = DataLoader(LocalTactileDataset(val_df, val_transform),
                            batch_size=BATCH_SIZE, shuffle=False, num_workers=4, pin_memory=True)
    test_loader = DataLoader(LocalTactileDataset(test_df, val_transform),
                             batch_size=BATCH_SIZE, shuffle=False, num_workers=4, pin_memory=True)
...
def upload_to_gcs(local_path, gcs_path):
    """Завантажити локальний файл в GCS"""
...
def train_model(arch, n_epochs=NUM_EPOCHS):
    print(f'\n{"=" * 50}')
    print(f'  {arch.upper()} | tactile_paving (2 кл.)')
    print(f'{"=" * 50}')
...
# ── Крок 6: Запуск циклу архітектур ───────────────────────────
...
for arch in ARCHS:
    ...
    # Збереження локально та вивантаження у хмару
    torch.save(model.state_dict(), local_save_path)
    upload_to_gcs(local_save_path, gcs_save_path)

    # Очищення оперативної пам'яті GPU
    del model
    torch.cuda.empty_cache()

# Експорт найкращих ваг як системного артефакту _best.pth під ASP.NET
if results:
...
# ── Крок 7: Результати та аналітика ──────────────────────────
if results:
    ...
    ax.set_title(f'{MODEL_NAME.capitalize()} Paving — порівняння моделей')
    ax.set_ylim(0, 1)
...
# Очищення локального диска інстансу від завантаженого датасету
shutil.rmtree(LOCAL_DATA_DIR, ignore_errors=True)
print('\n✅ Навчання повністю завершено!')
